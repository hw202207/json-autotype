...
valueSize (Object o) = (1+) . sum . map valueSize . Aeson.elems $ o
...
valueTypeSize (Object o) = (1+) . sum . map valueTypeSize . Aeson.elems $ o
...
valueDepth (Object o) = (1+) . maximum . (0:) . map valueDepth . Aeson.elems $ o
...
extractType (Object o)                  = TObj $ Dict $ Aeson.toHashMapText $ Aeson.map extractType o
...
typeCheck (Object d)    (TObj    e      ) = typeCheckKey `all` keysOfBoth
  where
    typeCheckKey :: Aeson.Key -> Bool
    typeCheckKey k = getValue k d `typeCheck` get (T.pack $ show k) e

    getValue   :: Aeson.Key -> Aeson.KeyMap Value -> Value
    getValue    k o = fromMaybe Null (Aeson.lookup k o)

    keysOfBoth :: [Aeson.Key]
    keysOfBoth  =  Set.toList $ Set.fromList $ Aeson.keys d ++ map (fromString . T.unpack) (Set.toList $ keys e)
